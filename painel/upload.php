		...
		public function salvar(){									
			...
			if (!move_uploaded_file($this->arquivo['tmp_name'], $destino)){
				...
				if ($this->ehImagem($extensao)){												
					...
					$this->redimensionar($largura, $altura, $tipo, $destino); //Inbdependente do tamanho da imagem ele irс redimensionar seja para mais ou para menos
...
/*	function up_arquivo_foto($images,$patch){
		...
		/*if(!empty($images)){ 
			...
			// Determinamos nossa largura mсxima permitida para a imagem
			//$upload->width = 250; 
			// Determinamos nossa altura mсxima permitida para a imagem
			//$upload->height = 250; 

			// Exibimos a mensagem com sucesso ou erro retornada pela funчуo salvar.
			//Se for sucesso, a mensagem tambщm щ um link para a imagem enviada.
			$upload->salvar();
